      ...
      <div className="w-full max-w-2xl bg-white rounded-lg shadow-lg p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-4 border rounded bg-white shadow-sm">
            <p className="font-semibold flex items-center gap-2"><FaUser /> Assigned To:</p>
            <p className="text-gray-700">{task.assignedTo || 'Not assigned'}</p>
          </div>
          <div className="p-4 border rounded bg-white shadow-sm">
            <p className="font-semibold flex items-center gap-2"><FaClock /> Deadline:</p>
            <p className="text-gray-700">{new Date(task.deadline).toLocaleDateString()}</p>
          </div>
          <div className="p-4 border rounded bg-white shadow-sm">
            <p className="font-semibold">Status:</p>
            <span className={`inline-block mt-1 px-3 py-1 rounded-full text-sm font-semibold ${getStatusBadge(task.status)}`}>
              {task.status}
            </span>
          </div>
          <div className="p-4 border rounded bg-white shadow-sm">
